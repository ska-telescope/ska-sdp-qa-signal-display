...
export class RfiStat {
  ...
  draw(data: { rfi_data: number[]; flags: number[] }): void {
    ...
    svg
      ...
      .attr('fill', (d, i) => {
        ...
        if (rfi_data[i] === 0 && flags[i] === 0) return FLAG_COLORS[1];
        if (rfi_data[i] === 0 && flags[i] === 1) return FLAG_COLORS[2];
        if (rfi_data[i] !== 0 && flags[i] === 0) return FLAG_COLORS[3];
        return 'black';
      });
